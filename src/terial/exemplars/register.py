...
def main(filepath, source_name):
    filepath = Path(filepath).resolve()
    image = imread(filepath)[:, :, :3]
    cropped_image = make_square_image(image, max_size=1000)
    vis.image(image.transpose((2, 0, 1)))
    vis.image(cropped_image.transpose((2, 0, 1)))

    data_dir = Path(config.BLOB_ROOT, 'exemplars')

    with database.session_scope() as sess:
        if sess.query(Exemplar).filter_by(source_path=str(filepath)).count() > 0:
            print(f'Exemplar with path {filepath} already exists.')
            return

        exemplar = Exemplar(source_path=str(filepath),
